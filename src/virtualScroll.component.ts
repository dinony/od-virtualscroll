import {
  ...
  ElementRef, Input, NgZone, OnDestroy, OnInit,
...
import {
  concatMap, debounceTime, distinctUntilChanged, filter,
  map, pairwise, partition, publish, scan, share, startWith,
  withLatestFrom
} from 'rxjs/operators'
...
export class VirtualScrollComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this._zone.runOutsideAngular(() => {
      this._subs.push(
        fromEvent(this._elem.nativeElement, 'scroll')
          .pipe(debounceTime(this.vsDebounceTime, animationScheduler))
          .subscribe(() => {
    ...
    const setScrollTopFunc$ = merge(userSetScrollTop$, focusRowSetScrollTop$, focusItemSetScrollTop$)
      .pipe(map((cmd: SetScrollTopCmd) => (state: IVirtualScrollState) => {
    ...
    this._subs.push(main$
      .pipe(filter(state => state.needsCheck && state.scrollWindow !== null))
      .subscribe(state => {
